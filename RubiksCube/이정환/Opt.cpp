...
int main(void)
{
	...
	while (1)
	{
		...
		//회전각 늘리기
		if (GetAsyncKeyState(VK_UP)) { X += 0.1; }
		if (GetAsyncKeyState(VK_DOWN)) { X -= 0.1; }
		if (GetAsyncKeyState(VK_LEFT)) { Y -= 0.1; }
		if (GetAsyncKeyState(VK_RIGHT)) { Y += 0.1; }
		if (GetAsyncKeyState(VK_SPACE)) { Z += 0.1; }
		if (GetAsyncKeyState(VK_CONTROL)) { Z -= 0.1; }
